// Generated from ABCMusic.g4 by ANTLR 4.0

package grammar;

import org.antlr.v4.runtime.tree.*;
import org.antlr.v4.runtime.Token;

public interface ABCMusicListener extends ParseTreeListener {
	void enterQuadruplet(ABCMusicParser.QuadrupletContext ctx);
	void exitQuadruplet(ABCMusicParser.QuadrupletContext ctx);

	void enterDefaultlength(ABCMusicParser.DefaultlengthContext ctx);
	void exitDefaultlength(ABCMusicParser.DefaultlengthContext ctx);

	void enterTempo(ABCMusicParser.TempoContext ctx);
	void exitTempo(ABCMusicParser.TempoContext ctx);

	void enterCommenttext(ABCMusicParser.CommenttextContext ctx);
	void exitCommenttext(ABCMusicParser.CommenttextContext ctx);

	void enterAbcline(ABCMusicParser.AbclineContext ctx);
	void exitAbcline(ABCMusicParser.AbclineContext ctx);

	void enterComposer(ABCMusicParser.ComposerContext ctx);
	void exitComposer(ABCMusicParser.ComposerContext ctx);

	void enterMeter(ABCMusicParser.MeterContext ctx);
	void exitMeter(ABCMusicParser.MeterContext ctx);

	void enterLyricword(ABCMusicParser.LyricwordContext ctx);
	void exitLyricword(ABCMusicParser.LyricwordContext ctx);

	void enterNotelengthdenominator(ABCMusicParser.NotelengthdenominatorContext ctx);
	void exitNotelengthdenominator(ABCMusicParser.NotelengthdenominatorContext ctx);

	void enterKey(ABCMusicParser.KeyContext ctx);
	void exitKey(ABCMusicParser.KeyContext ctx);

	void enterElement(ABCMusicParser.ElementContext ctx);
	void exitElement(ABCMusicParser.ElementContext ctx);

	void enterText(ABCMusicParser.TextContext ctx);
	void exitText(ABCMusicParser.TextContext ctx);

	void enterMultinote(ABCMusicParser.MultinoteContext ctx);
	void exitMultinote(ABCMusicParser.MultinoteContext ctx);

	void enterNumber(ABCMusicParser.NumberContext ctx);
	void exitNumber(ABCMusicParser.NumberContext ctx);

	void enterFieldnumber(ABCMusicParser.FieldnumberContext ctx);
	void exitFieldnumber(ABCMusicParser.FieldnumberContext ctx);

	void enterDigit(ABCMusicParser.DigitContext ctx);
	void exitDigit(ABCMusicParser.DigitContext ctx);

	void enterFieldkey(ABCMusicParser.FieldkeyContext ctx);
	void exitFieldkey(ABCMusicParser.FieldkeyContext ctx);

	void enterNotelengthnumerator(ABCMusicParser.NotelengthnumeratorContext ctx);
	void exitNotelengthnumerator(ABCMusicParser.NotelengthnumeratorContext ctx);

	void enterFielddefaultlength(ABCMusicParser.FielddefaultlengthContext ctx);
	void exitFielddefaultlength(ABCMusicParser.FielddefaultlengthContext ctx);

	void enterRest(ABCMusicParser.RestContext ctx);
	void exitRest(ABCMusicParser.RestContext ctx);

	void enterTuplet(ABCMusicParser.TupletContext ctx);
	void exitTuplet(ABCMusicParser.TupletContext ctx);

	void enterTriplet(ABCMusicParser.TripletContext ctx);
	void exitTriplet(ABCMusicParser.TripletContext ctx);

	void enterComment(ABCMusicParser.CommentContext ctx);
	void exitComment(ABCMusicParser.CommentContext ctx);

	void enterPitch(ABCMusicParser.PitchContext ctx);
	void exitPitch(ABCMusicParser.PitchContext ctx);

	void enterPunctuation(ABCMusicParser.PunctuationContext ctx);
	void exitPunctuation(ABCMusicParser.PunctuationContext ctx);

	void enterOtherfields(ABCMusicParser.OtherfieldsContext ctx);
	void exitOtherfields(ABCMusicParser.OtherfieldsContext ctx);

	void enterNotelengthmultiply(ABCMusicParser.NotelengthmultiplyContext ctx);
	void exitNotelengthmultiply(ABCMusicParser.NotelengthmultiplyContext ctx);

	void enterVoice(ABCMusicParser.VoiceContext ctx);
	void exitVoice(ABCMusicParser.VoiceContext ctx);

	void enterDuplet(ABCMusicParser.DupletContext ctx);
	void exitDuplet(ABCMusicParser.DupletContext ctx);

	void enterAbctune(ABCMusicParser.AbctuneContext ctx);
	void exitAbctune(ABCMusicParser.AbctuneContext ctx);

	void enterEndofline(ABCMusicParser.EndoflineContext ctx);
	void exitEndofline(ABCMusicParser.EndoflineContext ctx);

	void enterBasenote(ABCMusicParser.BasenoteContext ctx);
	void exitBasenote(ABCMusicParser.BasenoteContext ctx);

	void enterSpace(ABCMusicParser.SpaceContext ctx);
	void exitSpace(ABCMusicParser.SpaceContext ctx);

	void enterMidtunefield(ABCMusicParser.MidtunefieldContext ctx);
	void exitMidtunefield(ABCMusicParser.MidtunefieldContext ctx);

	void enterCharacter(ABCMusicParser.CharacterContext ctx);
	void exitCharacter(ABCMusicParser.CharacterContext ctx);

	void enterFieldcomposer(ABCMusicParser.FieldcomposerContext ctx);
	void exitFieldcomposer(ABCMusicParser.FieldcomposerContext ctx);

	void enterAbcmusic(ABCMusicParser.AbcmusicContext ctx);
	void exitAbcmusic(ABCMusicParser.AbcmusicContext ctx);

	void enterFieldvoice(ABCMusicParser.FieldvoiceContext ctx);
	void exitFieldvoice(ABCMusicParser.FieldvoiceContext ctx);

	void enterTitle(ABCMusicParser.TitleContext ctx);
	void exitTitle(ABCMusicParser.TitleContext ctx);

	void enterTupletnote(ABCMusicParser.TupletnoteContext ctx);
	void exitTupletnote(ABCMusicParser.TupletnoteContext ctx);

	void enterFieldmeter(ABCMusicParser.FieldmeterContext ctx);
	void exitFieldmeter(ABCMusicParser.FieldmeterContext ctx);

	void enterLyricbar(ABCMusicParser.LyricbarContext ctx);
	void exitLyricbar(ABCMusicParser.LyricbarContext ctx);

	void enterAccidental(ABCMusicParser.AccidentalContext ctx);
	void exitAccidental(ABCMusicParser.AccidentalContext ctx);

	void enterFieldtempo(ABCMusicParser.FieldtempoContext ctx);
	void exitFieldtempo(ABCMusicParser.FieldtempoContext ctx);

	void enterSongnumber(ABCMusicParser.SongnumberContext ctx);
	void exitSongnumber(ABCMusicParser.SongnumberContext ctx);

	void enterLyricunderscore(ABCMusicParser.LyricunderscoreContext ctx);
	void exitLyricunderscore(ABCMusicParser.LyricunderscoreContext ctx);

	void enterModeminor(ABCMusicParser.ModeminorContext ctx);
	void exitModeminor(ABCMusicParser.ModeminorContext ctx);

	void enterLyricstar(ABCMusicParser.LyricstarContext ctx);
	void exitLyricstar(ABCMusicParser.LyricstarContext ctx);

	void enterFieldtitle(ABCMusicParser.FieldtitleContext ctx);
	void exitFieldtitle(ABCMusicParser.FieldtitleContext ctx);

	void enterNotelengthfull(ABCMusicParser.NotelengthfullContext ctx);
	void exitNotelengthfull(ABCMusicParser.NotelengthfullContext ctx);

	void enterNotesline(ABCMusicParser.NoteslineContext ctx);
	void exitNotesline(ABCMusicParser.NoteslineContext ctx);

	void enterOctave(ABCMusicParser.OctaveContext ctx);
	void exitOctave(ABCMusicParser.OctaveContext ctx);

	void enterBarline(ABCMusicParser.BarlineContext ctx);
	void exitBarline(ABCMusicParser.BarlineContext ctx);

	void enterNotelength(ABCMusicParser.NotelengthContext ctx);
	void exitNotelength(ABCMusicParser.NotelengthContext ctx);

	void enterLyric(ABCMusicParser.LyricContext ctx);
	void exitLyric(ABCMusicParser.LyricContext ctx);

	void enterNotelengthhalf(ABCMusicParser.NotelengthhalfContext ctx);
	void exitNotelengthhalf(ABCMusicParser.NotelengthhalfContext ctx);

	void enterAbcheader(ABCMusicParser.AbcheaderContext ctx);
	void exitAbcheader(ABCMusicParser.AbcheaderContext ctx);

	void enterKeyaccidental(ABCMusicParser.KeyaccidentalContext ctx);
	void exitKeyaccidental(ABCMusicParser.KeyaccidentalContext ctx);
}